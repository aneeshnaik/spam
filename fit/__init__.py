...
import numpy as _np
from scipy.constants import G as _G, parsec as _pc
from .prior import lnprior as _lnprior
from .likelihood import lnlike as _lnlike

# physical constants and cosmology; all SI units
_delta = 93.6
_h = 0.7
_H0 = _h*100*1000/(1e+6*_pc)
_Msun = 1.989e+30
    ...
    **MG_grid_dim : int, {1, 2}
    ...
    def __init__(self, galaxy, nwalkers=30, ntemps=4, **kwargs):
        ...
        if self.upsilon == 'single':
            ndim += 1
            lb = _np.append(lb, -0.52)
            ub = _np.append(ub, -0.1)
            theta_dict['ML_disc'] = 2
            theta_dict['ML_bulge'] = 2
        elif self.upsilon == 'double':
            ndim += 2
            lb = _np.append(lb, [-0.52, -0.52])
            ub = _np.append(ub, [-0.1, -0.1])
            theta_dict['ML_disc'] = 2
        ...
        if 'fR_parameter' in self.__dict__.keys():
            if self.fR_parameter == 'free':
                ndim += 1
                lb = _np.append(lb, -9)
                ub = _np.append(ub, _np.log10(2e-6))
                theta_dict['fR0'] = ndim-1
        if self.infer_errors:
            ndim += 1
            lb = _np.append(lb, 0)
            ub = _np.append(ub, 2*galaxy.v_err.max())
        ...
        if 'model' in kwargs.keys():
            ...
            if self.fR is True:
                ...
                if self.model[0] not in ['H', 'I']:
                    ...
                    v = _np.linspace(_np.log10(1.563e-8), _np.log10(2e-6), 20)
                    self.log10fR0 = v[ind]
    ...
    def iterate(self, niter, threads=1, initial_run=True, print_progress=True):
        ...
        kwargs = {'halo_type': self.halo_type, 'upsilon': self.upsilon,
                  'infer_errors': self.infer_errors,
                  'baryon_bound': self.baryon_bound,
                  'scaling_priors': self.scaling_priors,  'fR': self.fR,
        ...
        if initial_run:
            ...
            self.chain = _np.dstack((self.chain, sampler.chain))
            self.lnprob = _np.dstack((self.lnprob, sampler.lnprobability))
        return
