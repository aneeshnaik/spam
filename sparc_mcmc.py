...
Created: 2018
Author: A. P. Naik
Description: Code to perform MCMC fitting on SPARC galaxies to fit for f(R)
upturns.
"""
import sys
import time
import emcee
import numpy as np
from sparc_v_pipeline import v_circ_pipeline
from sparc_priors import lnprior
...
class SoloGalFit:
    ...
    def __init__(self, galaxy, fR_parameter, log10fR0=None, Rscr_fixed=None,
                 halo_type='NFW', StellarScreening=False, EnvScreening=False,
                 ML_ratio='single', CosmicBaryonBound=True,
                 scaling_priors=True, infer_errors=True, MGGridDim=None,
                 Verbose=False, ntemps=5,
                 nwalkers=20, threads=4):
        """
    ...
    def iterate(self, niter, initial_run=True, p0=None):
        """
        ...
        sampler = emcee.PTSampler(self.ntemps, self.nwalkers, self.ndim,
                                  lnlike, lnprior, threads=self.threads,
    ...
    def converge(self, niter):
        ...
        v_fit = v_circ_pipeline(theta, self.theta_dict, self.galaxy,
...
def lnlike(theta, theta_dict, galaxy, fR_parameter, log10fR0, Rscr_fixed,
    ...
    model = v_circ_pipeline(theta=theta, theta_dict=theta_dict, galaxy=galaxy,
